...
function Avatar({ src = "", alt = "avatar", size = "40px", shadow = true }) {
  // js 자리
...
- Skeleton 샘플 : https://cssgradient.io/
...
- `JS 자리`에 if 문과 switch 문을 사용할 수 있다.

- `JSX 자리`에 if 문과 switch 문을 사용할 수 없다.
- if 문과 switch 문 대신에 `3 항 연산자`가 가장 많이 사용됨.
- `JSX 자리`에 falshy 한 값은 출력이 안됨.
