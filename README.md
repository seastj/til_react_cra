...
- 한국어로는 `컴포넌트에서 같이 실행한다`
- 컴포넌트의 state 와 lifecycle 에 따라서 같이 실행되는 함수
- useState, useEffect, useRef, useMemo, useCallback...
- 개발자도 리액트의 훅처럼 생성이 가능함. (많이 만들어서 활용함)

## 2. Hook 을 만들 때 유의사항

- 동일한 기능을 여러 번 사용한다면 함수를 만든다.
- 여러 번 활용되는 함수가 만약 컴포넌트에 사용된다면 hook 을 생성한다.

## 3. 폴더 구조 및 함수명 주의

- 일반적으로 `/src/hooks` 폴더를 생성
- 파일명은 반드시 접두사로 `use함수명.js` 으로 생성해야 리액트에서 인식함.
- 반드시 컴포넌트함수 안쪽에 배치를 하여야 한다.
- if 문 또는 for 문의 안쪽에 배치하지 않아야 작동한다.
