...
- 리액트는 각각 컴포넌트를 위한 state 가 있다. (useState)
- 리액트는 글로벌 state 가 있다. (Context)
...
function ChoiceCharacter({ userInfo }) {
  return <div>{userInfo.userName}캐릭터 선택 서비스</div>;
}
...
- /src/contexts/UserThemeContext.jsx 파일생성
...
function Point() {
  // 글로벌 리액트 변수인 context 에 접근하였다.
  const { userInfo } = useContext(UserInfoContext);
  const { bucketList, setBuketList } = useContext(UserBucketContext);
  const handleClick = _goodId => {
    setBuketList([...bucketList, _goodId]);
  };
  return (
...
function ColorComponent() {
  ...
  return (
    <div style={{ backgroundColor: color, height: "100vh" }}>
      <h1>배경 색상변경하기</h1>
      <button onClick={e => dispatch({ type: "YELLOW" })}>노란색</button>
      <button onClick={e => dispatch({ type: "SKYBLUE" })}>하늘색</button>
      <button onClick={e => dispatch({ type: "RESET" })}>초기화</button>
    </div>
  );
}
...
export const ThemeContextProvider = ({ children }) => {
  return <ThemeContextProvider value={0}>{children}</ThemeContextProvider>;
};
```
...
export const ThemeContextProvider = ({ children }) => {
  ...
  return (
    <ThemeContextProvider value={{ theme, dispatch }}>
      {children}
